...
%----------------------- DO NOT OVERRIDE -----------------------
strengthB([], _, _, Rand) :-
    noise_level(Level), random(0, Level, Number),
    Rand is Number. % Add random value to avoid deadlock
%---------------------------------------------------------------
% ++++++++++++++++++++++++++++++++ playerB Code ++++++++++++++++++++++++++++++++
% Strength assesses utility of the current game state for player based on its Color
...
strengthB([piece(_, Color, Type)|Board], Color, OppositeColor, Strength) :-
    ...
    Strength is PartialStrength + Value, !.
strengthB([piece(_, OppositeColor, Type)|Board], Color, OppositeColor, Strength) :-
    valueB(Type, Value),
    strengthB(Board, Color, OppositeColor, PartialStrength),
    Strength is PartialStrength - Value.
...
sufficientB(Player, Board, Turn, Moves, Depth, Alpha, Beta, Move, Val, Move, Val) :-
    Player = Turn, % It is the Player turn to play, MAX node at Turn
    Val > Beta, !. % Pruning the branch since it is not useful
sufficientB(Player, Board, Turn, Moves, Depth, Alpha, Beta, Move, Val, BestMove, BestVal) :-
    new_bounds(Player, Turn, Alpha, Beta, Val, NewAlpha, NewBeta),
    find_best(Player, Board, Turn, Moves, Depth, NewAlpha, NewBeta, Move1, Val1),
    better_of(Player, Turn, Move, Val, Move1, Val1, BestMove, BestVal).
% Code to collect moves given the current state Board
...
alpha_beta(Player, Board, Turn, Depth, Alpha, Beta, BestMove, MoveVal) :-
    ...
    (player(Player, white) -> 
        ...
        BestMove, MoveVal);
        MoveVal is -32000)).
find_best(Player, Board, Turn, [move(From, To)|Moves], Depth, Alpha, Beta, BestMove, BestVal) :-
    make_move(Board, From, To, NewBoard),
...
drawSymbol(Symbol, N) :-
    ...
    write(Symbol),
    NextN is N - 1,
    drawSymbol(Symbol, NextN).
    draw_top_bot_border(0) :- drawSymbol('+', 1).
    draw_top_bot_border(Col) :- 
    drawSymbol('+----', 1),
    NextCol is Col - 1,
    draw_top_bot_border(NextCol).

draw_content_with_number(Board, Row, Col) :- drawSymbol(Row, 1), draw_content(Board, Row, Col).

draw_content(Board, Row, 0) :-
    drawSymbol('|', 1), nl.
draw_content(Board, Row, Col) :-
    Col > 0,
    draw_cell(Board, Row, Col),
    NextCol is Col - 1,
    draw_content(Board, Row, NextCol).

draw_cell(Board, Row, Col):- 
    pair(Letter, Col),
    pair(Piece, ShortPiece),
    mymember(piece(Letter-Row, Color, Piece), Board),
    ((Color = black, drawSymbol('|', 1), drawSymbol(' ', 1), drawSymbol('*', 1), drawSymbol(ShortPiece, 1), drawSymbol(' ', 1));
     (Color = white,drawSymbol('|', 1), drawSymbol(' ', 1), drawSymbol(' ', 1), drawSymbol(ShortPiece, 1), drawSymbol(' ', 1))).
...
draw_bottom_letters(0) :- nl.
draw_bottom_letters(Col) :-
    Col > 0,
    pair(Letter, Col),
    drawSymbol(' ', 2),
    drawSymbol(Letter, 1),
    drawSymbol(' ', 2),
    NextCol is Col - 1,
    draw_bottom_letters(NextCol).


draw_board(Board, 0,Col) :- drawSymbol(' ', 1), draw_top_bot_border(Col), nl, drawSymbol(' ', 2), draw_bottom_letters(Col).
draw_board(Board, Row,Col) :-
    Row > 0,
    drawSymbol(' ', 1),
    draw_top_bot_border(Col),
    nl,
    draw_content_with_number(Board, Row, Col),
    NextRow is Row - 1,
    draw_board(Board, NextRow, Col).
